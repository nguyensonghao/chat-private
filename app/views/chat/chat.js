...
.controller('ChatController', ["$rootScope", "$scope", "$state", "localstoreServ", "authServ", "maziiServ", "dictUtilSer",
    ...
    $scope.changeTab = function (index) {
        ...
        if (index == 1) {
            ...
            $scope.titleBar = 'Danh sách tin nhắn';
            $scope.newMessage = false;
        } else {
            $scope.tab_3 = 'menu-active';
            $scope.tab_2 = $scope.tab_1 = '';
            $scope.titleBar = 'Danh sách người dùng';
            $scope.tabScreen = 7;
    ...
    $scope.changeScreen = function (index) {
        ...
        switch(index) {
            case 0:
                ...
                break;
            default:
                $scope.titleBar = 'Nhóm nhóm JLPT ' + index;
        }
    }
    ...
    socket.on('login-success', function (data) {
        localstoreServ.setItem('user', data);
        $rootScope.user = data;
        $scope.user = data;
        user = data;
        getDataWhenLogined();
    ...
    $scope.redirectChatPrivate = function (user) {
        ...
        $scope.titleBar = 'Chat với ' + user.username;
        $scope.newMessage = false;
        $scope.tabScreen = 6;
        var data = {
            userSend : $scope.user,
